...
public:
	...
	void set_tet_ref(const int t, const std::array < utilities::vec3, 4> tet_ref);
	void set_tet_ref(const std::vector<std::array < utilities::vec3, 4> > tets_ref);
	bool go();
...
public:
	...
	std::vector<std::array<utilities::vec3, 4>> refs_grad_;


	// intern variables 
	std::vector<utilities::mat3x3> J_; // per-tet Jacobian matrix = [[JX.x JX.y, JX.z], [JY.x, JY.y, JY.z], [JZ.x, JZ.y, JZ.z]]
	std::vector<utilities::mat3x3> K_; // per-tet dual basis: det J = dot J[i] * K[i]
	std::vector<double> det_; // per-tet determinant of the Jacobian matrix
