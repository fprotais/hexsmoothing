...
public:
	...
	vertex_smoother(utilities::HexahedralMesh& m);
	...
	*/
	void set_bnd_triangles(const utilities::TriangleMesh& bnd);
	void set_features_segment(const utilities::CurveMesh& ft);
	...
	void set_vertex_point(const int i, const utilities::vec3 v);
...
private:
	...
	utilities::HexahedralMesh& _m;

	std::vector<utilities::vec3>& _pts;
	struct VertData {
		bool locked = false;
	...
	struct TetData {
		...
		std::array<utilities::vec3, 4> pre_computed;
	};
	...
	struct ProjectData {
		utilities::TriangleMesh bnd;
		utilities::CurveMesh ft;
		std::vector<utilities::vec3> pts;
		std::vector<std::vector<int>> vert2triangles;
		std::vector<std::vector<int>> vert2segments;
		std::vector<int> vert2point;
	} _proj;

	utilities::vec3 projected_position(int v) const;
	...
	utilities::vec3 compute_vert_elliptic_grad_trunc_hess(int v, utilities::mat3x3& hess) const;
	utilities::vec3 compute_naive_laplacian_direction(int v) const;
