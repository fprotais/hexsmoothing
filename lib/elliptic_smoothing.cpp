...
void Elliptic_smoother_3D::set_tet_ref(const int t, const std::array < vec3, 4> tet_ref) {
    assert(t < N_tets_);
    mat3x3 M = { tet_ref[1] - tet_ref[0], tet_ref[2] - tet_ref[0], tet_ref[3] - tet_ref[0] };
...
void Elliptic_smoother_3D::set_tet_ref(const std::vector<std::array < vec3, 4> > tets_ref) {
...
void Elliptic_smoother_3D::init() {
    ...
    FOR(t, N_tets_) {
        ...
        refs_grad_[t] = { vec3(-a,-a,-a), vec3(a,a,-a), vec3(-a,a,a), vec3(a,-a,a) };
        vol_[t] = 1./6.*a;
    }
    J_.resize(N_tets_);
    K_.resize(N_tets_);
...
void Elliptic_smoother_3D::evaluate_jacobian() {
    ...
    ninverted_ = 0;
    FOR(t, N_tets_) {
        J_[t] = { vec3(0,0,0), vec3(0,0,0), vec3(0,0,0) };
        FOR(i, 4) FOR(d, 3)
...
void Elliptic_smoother_3D::standard_elliptic_energy(const std::vector<double>&, double& F, std::vector<double>& G) {
    ...
    FOR(t, N_tets_) {
        ...
        FOR(d, 3) {
            vec3 dfda = J_[t][d] * (2. / c2) - K_[t][d] * ((2. * f * c3) / (3. * c1));
            vec3 dgda = K_[t][d] * ((2 * det_[t] - g * c3) / c1);
